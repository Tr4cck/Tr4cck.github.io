...
题目使用**或非指令**构建了一个单指令虚拟机, 因为一个或非就能构成全功能集合, 可以实现与或非, 进而实现单比特全加器, 进而实现多比特全加器, 进而实现各种数学运算; 条件分支也很好实现. 虚拟机字长 2 字节, 内存模型为线性 65536 字节, 即刚好可以在一条指令内直接寻址完所有的地址空间.
核心逻辑如下:
...
那么我们其实就可以开始分析, 比如 `mem[7]`、`mem[3]` 之类的作用了, 分析结果见上面的源码注释.
